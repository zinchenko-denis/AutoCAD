...
> **Статус (Денис, 23.09): находки сохранены как бэклог «на потом»; в
> работе — фидбэк Германа (русты ATTILE, окно ATFRAME). Фокус — фасадные
> модули; светопрозрачные (ABlockGen, ATableSpec) пока не в приоритете.**
> Дополнение по ходу работ: у окна ближе 100 мм к боковому краю зоны
> оконная стойка ATFRAME (грань + 100) ставится за краем зоны.

> **24.09:** получена независимая рецензия того же кода; её фасадные замечания
> воспроизведены и исправлены — см. `docs/REVIEW_RESPONSE_24.09.md`.

## Исправлено 23.09 — фасадные модули (выложено сборкой №22, build-95)

Денис: «явные ляпы — исправить, пока Герман тестирует №21»; светопрозрачные
(ABlockGen, ATableSpec) не трогали.

| Что было | Что стало | Проверка |
|---|---|---|
| ATTILE по штриховке/марке зоны отказывал «нет пригодных зон» | геометрия из `_fzones.json`, полилинии зоны не задваиваются, ЗАХВАТКА = имя зоны | вошло в №21; xmod X1 OK |
| ATFRAME не читал зоны ATFZONE (ждал `zone.contour`) | читает facade_zone/1 (outer/openings, мм/м; дуги — нота) | ZF1–ZF4, xmod X8 OK |
| Г/ступени/П-стена и фронтон: подсистема по ГАБАРИТУ, треть элементов в воздухе | стойки, ГП/НГП, кронштейны, кляммеры — по контуру стены, верх — по скату/уступу | PG1–PG2; синтетика: за стеной 0 (было 480/480 на сид) |
| Оси швов общим списком: в зоне стойки по швам соседней (14 → 29) | ATCLAD пишет в метку оси своей зоны, ATFRAME шлёт каждой зоне её оси | ZF6, xmod X3 OK |
| Штриховка + полилинии зоны — задвоение (после починки чтения зон) | полилинии зоны исключаются, как в ATCLAD | xmod X4 OK |
| Зона в «кармане» Г-стены у ATFRAME считалась проёмом | группировка контуров точкой в полигоне, как у раскладки | ZF5, xmod X7 OK |
| Оконная стойка у окна ближе 100 мм к краю — за краем зоны | не ставится (нота) | PG3 |
| СП-60-40 межэтажной сквозь соседнее окно / вдоль окна под ним | режется соседними проёмами и стеной | PG4 |
| ATFZONE: проём с нахлёстом за стену — молча «фантом-зона» | ошибка E_CONTOUR_CROSSES с именами контуров | TestReview2309; zones_synth — нарушений нет |
| Отлив/откос круглых окон «гулял» до 17 см от положения на чертеже | хорда на 45° — пополам; расхождение ≤ 0,8 мм | zones_synth |
| Зона в метрах: допуски 0,5 м вместо 0,5 мм | допуски в мм | TestReview2309 |
| Движок без срока — зависший exe вешал AutoCAD | срок 120/600/300 с (зоны/раскладка/подсистема), процесс останавливается | CI #162 (компиляция) |
| В CI не гонялись расчёт AFrame (79) и CLI (8) | добавлены в check.yml | CI #162 |

![было/стало: подсистема по контуру стены](review_2309/afr_contour_fix.png)

Прямоугольные стены — без изменений (D-сверка полигона Германа байт в байт,
краш-аудит 181/0). Осталось по фасадам: правило для куска вертикали
ортогональной выше последнего ГП (вопрос Герману), частичная кромка окна на
границе зоны (вопрос), размер метки ATTILE (не ошибка — оптимизация).
