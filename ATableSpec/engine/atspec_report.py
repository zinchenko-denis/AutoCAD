...
def _beads_expand(records: List[dict], cfg) -> List[dict]:
    ...
    cfg = {"layer": "<слой стоек>", "source": "<слой заполнений>"} (def-ключ "beads";
    нет ключа — записи нетронуты, старые таблицы не знают о штапиках).
    Для каждой записи НЕ со слоя стоек:
      ...
      стойкой, ШТ_ЗАЗОР = ширина зазора.
    Записи-«половины» (слой ШТАПИК-РАЗРЕЗ, поля ШТ_РАЗМЕР и ШТ_ЧАСТЬ=«низ»/«верх»)
    создаются ТОЛЬКО от записей слоя source — иначе стык стоек режет и ригеля,
    и кронштейны, и всё прочее с габаритом, замусоривая секцию разрезных штапиков
    (краш-тест 07.07: на реальном чертеже мусор 1/171 мм от ригелей). Нет source —
    клонов нет вовсе (консервативно). Y-интервалы левой/правой колонок стоек
    сливаются (дубли схлопнуты). Запись без габарита получает ШТ_СТЫК=0 (фолбэк)."""
    ...
    out = list(records)
    for r in records:
        rlay = str(r.get("layer", "")).strip().lower()
        if rlay == lay:
            continue
        ...
        if not src or rlay != src:      # половины — только от слоя-источника
            continue
