...
# ───────── МНОГОСЛОЙНЫЕ ФИЛЬТРЫ (ТЗ 08.07): values[] в фильтре ─────────
def _R(layer, **at): return {"name": "blk", "layer": layer, "attributes": {k: str(v) for k, v in at.items()}}
_mf = [_R("RF-стойки",  МАРКИРОВКА="С1",  Длина=3495),
       _R("RF-стойки",  МАРКИРОВКА="С2",  Длина=3495),
       _R("RF-ригеля",  МАРКИРОВКА="Р1",  Длина=1130),
       _R("RF-створки", МАРКИРОВКА="Ств1", Длина=900),
       _R("0",          МАРКИРОВКА="X0",  Длина=1),
       _R("RF-заполнения", МАРКИРОВКА="Сп1", Длина=500),
       _R("RF-заполнения", МАРКИРОВКА="Вр1", Длина=500),
       _R("RF-заполнения", МАРКИРОВКА="Бг1", Длина=500)]
def _rows(flt, cols=("=Object.«МАРКИРОВКА»",)):
    d = {"sections": [{"header": ["М"], "columns": list(cols), "filter": flt}]}
    return run_report(_mf, d)["sections"][0]["rows"]
_m = lambda rows: sorted(r[0] for r in rows)
# U1: «=» со списком — любое из
assert _m(_rows([{"field": "МАРКИРОВКА", "op": "=", "values": ["Сп1", "Вр1"]}])) == ["Вр1", "Сп1"]
# U2: «содержит» со списком — любое из
assert _m(_rows([{"field": "МАРКИРОВКА", "op": "содержит", "values": ["Сп", "Вр"]}])) == ["Вр1", "Сп1"]
# U3: «≠» со списком — ни одно из
assert _m(_rows([{"field": "Слой", "op": "≠", "values": ["0", "RF-заполнения"]}])) == ["Р1", "С1", "С2", "Ств1"]
# U4: «не содержит» со списком — ни одно из
assert _m(_rows([{"field": "МАРКИРОВКА", "op": "не содержит", "values": ["Вр", "Бг"]},
                 {"field": "Слой", "op": "=", "value": "RF-заполнения"}])) == ["Сп1"]
# U5: КНФ — строки «И», список внутри строки «ИЛИ»
assert _m(_rows([{"field": "Слой", "op": "=", "values": ["RF-стойки", "RF-ригеля"]},
                 {"field": "Длина", "op": "=", "value": "3495"}])) == ["С1", "С2"]
# U6: мультиисточник Слой=[...] — сквозная нумерация и группа по общему полю
_u6 = run_report(_mf, {"sections": [{"header": ["№", "М", "К"],
    "columns": ["=row", "=Object.«МАРКИРОВКА»", "=Count"],
    "filter": [{"field": "Слой", "op": "=", "values": ["RF-стойки", "RF-ригеля", "RF-створки"]}],
    "group_by": 1, "sort_by": [1, "asc"]}]})["sections"][0]["rows"]
assert len(_u6) == 4 and [r[0] for r in _u6] == [1, 2, 3, 4], _u6      # 4 группы, № сквозные
assert [r[1] for r in _u6] == ["Р1", "С1", "С2", "Ств1"], _u6          # натуральная сортировка марок
# U7: исключение слоёв — пустой источник + Слой ≠ список
assert _m(_rows([{"field": "Слой", "op": "≠", "values": ["0", "RF-заполнения", "RF-створки"]}])) \
       == ["Р1", "С1", "С2"]
# U8: диапазонный оператор со списком — защитно берётся первое значение
assert _m(_rows([{"field": "Длина", "op": ">", "values": ["1000", "1"]}])) == ["Р1", "С1", "С2"]
# U9: пробелы вокруг элементов чистятся, пустые отбрасываются
assert _m(_rows([{"field": "МАРКИРОВКА", "op": "=", "values": [" Сп1 ", "", "  ", "Р1"]}])) == ["Р1", "Сп1"]
# U10: СТАРЫЙ путь — value-строка с «;» НЕ режется (семантика старых def неприкосновенна)
assert _rows([{"field": "МАРКИРОВКА", "op": "=", "value": "Сп1;Вр1"}]) == []
# U11: values имеет приоритет над value; values пустой после чистки -> откат на value
assert _m(_rows([{"field": "МАРКИРОВКА", "op": "=", "values": ["Р1"], "value": "Сп1"}])) == ["Р1"]
