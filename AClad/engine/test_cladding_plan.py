# -*- coding: utf-8 -*-
"""Юниты фазы 2 (облицовка): cladding_plan. Запуск: PYTHONUTF8=1
python3 test_cladding_plan.py. Геометрия проверяется руками посчитанными
раскладками (docs/CLADDING.md)."""
import sys
from cladding_plan import cladding_plan

_n = 0


def ok(cond, msg):
    global _n
    _n += 1
    if not cond:
        print("FAIL:", msg)
        sys.exit(1)


def near(a, b, t=1e-6):
    return abs(a - b) <= t


def rect(x0, y0, x1, y1):
    return [[x0, y0], [x1, y0], [x1, y1], [x0, y1]]


def plan(**kw):
    # min_cut НЕ задаётся: тесты работают на боевом дефолте 150 (В4)
    base = {"tile": {"w": 600, "h": 600}, "gap": {"v": 10, "h": 10},
            "datum": 0.0, "mode": "edge"}
    base.update(kw)
    return cladding_plan(base)


# ── C1: прямоугольник 3040×1210, руст 10 — ровно 5×2 целых ──
p = plan(contours=[{"outer": rect(0, 0, 3040, 1210)}])
ok(p["ok"] and p["summary"]["tiles"] == 10 and p["summary"]["cut"] == 0,
   "C1: 5×2 целых (%s)" % p["summary"])
ok(sorted({t["x"] for t in p["inserts"]}) == [0, 610, 1220, 1830, 2440] and
   sorted({t["y"] for t in p["inserts"]}) == [0, 610],
   "C1: сетка от края и от датума")

# ── C2: датум 300 — ниже не кладём (note), верхний ряд подрезан ──
p = plan(contours=[{"outer": rect(0, 0, 610, 1210)}], datum=300.0)
ys = sorted({(t["y"], t["h"]) for t in p["inserts"]})
ok(ys == [(300.0, 600.0), (910.0, 300.0)],
   "C2: ряды от датума, верх подрезан (%s)" % ys)
ok(any("ниже отметки старта на 300" in n for n in p["notes"]),
   "C2: note про низ")

# ── C3: окно в среднем ряду (edge): примыкающие камни отступают от
#    окна на верт. руст (П4, 21.07) — подрезки 480/520, не 490/530 ──
p = plan(contours=[{"outer": rect(0, 0, 3040, 1820),
                    "holes": [rect(1100, 610, 1900, 1210)]}])
mid = sorted([t for t in p["inserts"] if near(t["y"], 610)],
             key=lambda t: t["x"])
ok(len(mid) == 4, "C3: средний ряд 4 камня (%d)" % len(mid))
ok(near(mid[1]["x"], 610) and near(mid[1]["w"], 480) and
   near(mid[2]["x"], 1910) and near(mid[2]["w"], 520),
   "C3: подрезки у окна 480/520 со швом (%s)"
   % [(t["x"], t["w"]) for t in mid])
ok(all(near(t["w"], 600) for t in p["inserts"] if not near(t["y"], 610)),
   "C3: крайние ряды целые")

# окно, выровненное ЛЕВОЙ гранью в сетку и по рядам: слева и по
# вертикали швы совпали с сеткой (целые), справа от окна камень
# столбца поджат швом окна → 590 (П4)
p = plan(contours=[{"outer": rect(0, 0, 3040, 1820),
                    "holes": [rect(1220, 610, 1830, 1210)]}])
ok(p["summary"]["cut"] == 1 and p["summary"]["tiles"] == 14,
   "C3b: окно в сетке — один поджатый шов справа (%s)" % p["summary"])
c3b = [t for t in p["inserts"] if near(t["y"], 610) and t["x"] > 1500]
ok(near(c3b[0]["x"], 1840) and near(c3b[0]["w"], 590),
   "C3b: камень справа от окна 1840/590 (%s)"
   % [(t["x"], t["w"]) for t in c3b])

# ── C4: «от проёмов» — двери во всю высоту ряда; камни отступают от
#    дверей на шов (П4); простенок между швами 1380: остаток 160 <
#    300 → В5-B: целая в середине, подрезки (160+600)/2=380 ──
holes4 = [rect(200, 0, 800, 600), rect(2200, 0, 2800, 600)]
p = plan(contours=[{"outer": rect(0, 0, 3000, 600), "holes": holes4}],
         mode="openings")
row = sorted(p["inserts"], key=lambda t: t["x"])
ok([(t["x"], t["w"]) for t in row] ==
   [(0.0, 190.0), (810.0, 380.0), (1200.0, 600.0), (1810.0, 380.0),
    (2810.0, 190.0)],
   "C4: швы вокруг дверей, целая в центре простенка (%s)"
   % [(t["x"], t["w"]) for t in row])

# ── C5 (В2+П4): ряд НАД проёмами — анкерные швы дверей действуют и
#    там (сквозные вертикальные русты); над дверью камень ровно в
#    пролёте между швами; раскладка симметрична ──
p = plan(contours=[{"outer": rect(0, 0, 3000, 1210), "holes": holes4}],
         mode="openings")
top = sorted([t for t in p["inserts"] if near(t["y"], 610)],
             key=lambda t: t["x"])
ok([(t["x"], t["w"]) for t in top] ==
   [(0.0, 190.0), (200.0, 600.0), (810.0, 380.0), (1200.0, 600.0),
    (1810.0, 380.0), (2200.0, 600.0), (2810.0, 190.0)],
   "C5: сквозные швы от граней дверей (%s)"
   % [(t["x"], t["w"]) for t in top])
ok([(t["x"], t["w"]) for t in top] ==
   [(3000.0 - t["x"] - t["w"], t["w"]) for t in reversed(top)],
   "C5: раскладка симметрична относительно середины фасада")
# швы между соседними камнями ряда — НЕ меньше руста (стыков впритык
# больше нет: П4 закрыл щель «камень-в-камень» на анкерных линиях)
gaps5 = [round(top[i + 1]["x"] - (top[i]["x"] + top[i]["w"]), 6)
         for i in range(len(top) - 1)]
ok(all(g >= 10 - 1e-6 for g in gaps5),
   "C5b: все стыки ряда ≥ верт. руста (%s)" % gaps5)

# ── C6: остаток меньше min_cut — камень не ставится, note ──
p = plan(contours=[{"outer": rect(0, 0, 615, 600)}])   # остаток 5 < 20
ok(p["summary"]["tiles"] == 1 and
   any("< min_cut" in n for n in p["notes"]),
   "C6: 5 мм не ставим (%s)" % p["summary"])

# ── C7: ступень контура НЕ кратная ряду — подрезка по высоте сама ──
step7 = [[0, 0], [2440, 0], [2440, 900], [1220, 900], [1220, 1210],
         [0, 1210]]
p = plan(contours=[{"outer": step7}])
r1 = [t for t in p["inserts"] if near(t["y"], 0)]
lo = [t for t in p["inserts"] if near(t["y"], 610) and near(t["h"], 290)]
hi = [t for t in p["inserts"] if near(t["y"], 900) and near(t["h"], 310)]
ok(len(r1) == 4 and all(near(t["h"], 600) for t in r1),
   "C7: нижний ряд 4 целых")
ok(len(lo) == 4 and len(hi) == 2,
   "C7: ряд 2 — подполосы 290 (вся ширина, y=610) и 310 (левая часть, "
   "y=900): %d/%d" % (len(lo), len(hi)))

# ── C8: неортогональный контур — пропуск с note ──
p = plan(contours=[{"outer": [[0, 0], [1000, 0], [1000, 800], [100, 900]]},
                   {"outer": rect(0, 0, 610, 600)}])
ok(p["summary"]["tiles"] == 1 and
   any("неортогональ" in n for n in p["notes"]),
   "C8: кривой контур пропущен, второй обработан")

# ── C9: два контура — ОБЩИЙ ГОРИЗОНТ (сетка рядов одна) ──
p = plan(contours=[{"outer": rect(0, 0, 610, 1210)},
                   {"outer": rect(5000, 305, 5610, 1210)}])
y2 = sorted({t["y"] for t in p["inserts"] if t["x"] >= 5000})
ok(y2 == [305.0, 610.0],
   "C9: контур 2 стартует подрезкой 305 и попадает в горизонт (%s)" % y2)
ok(near([t for t in p["inserts"]
         if t["x"] >= 5000 and near(t["y"], 305)][0]["h"], 295),
   "C9: стартовый камень контура 2 подрезан до 295")

# ── C10: нулевой руст — плотная укладка ──
p = plan(contours=[{"outer": rect(0, 0, 1800, 600)}], gap={"v": 0, "h": 0})
xs = sorted(t["x"] for t in p["inserts"])
ok(xs == [0.0, 600.0, 1200.0] and p["summary"]["cut"] == 0,
   "C10: руст 0 — стык в стык (%s)" % xs)

# ── C11: «от проёмов» без проёмов вовсе == «от края» интервала;
#    с origin.x — тоже совпадают (оба от точки привязки) ──
pa = plan(contours=[{"outer": rect(0, 0, 3040, 600)}], mode="openings")
pb = plan(contours=[{"outer": rect(0, 0, 3040, 600)}], mode="edge")
ok([(t["x"], t["w"]) for t in pa["inserts"]] ==
   [(t["x"], t["w"]) for t in pb["inserts"]],
   "C11: без проёмов режимы совпадают")
pa = plan(contours=[{"outer": rect(0, 0, 3040, 600)}], mode="openings",
          origin={"x": 305.0, "y": 0.0})
pb = plan(contours=[{"outer": rect(0, 0, 3040, 600)}], mode="edge",
          origin={"x": 305.0, "y": 0.0})
ok([(t["x"], t["w"]) for t in pa["inserts"]] ==
   [(t["x"], t["w"]) for t in pb["inserts"]],
   "C11b: и с точкой привязки совпадают")

# ── C13 (В4): дефолт min_cut = 150 — остаток 90 пропускается ──
p = plan(contours=[{"outer": rect(0, 0, 700, 600)}])
ok(p["summary"]["tiles"] == 1 and
   any("90 < min_cut" in n for n in p["notes"]),
   "C13: остаток 90 не ставится на дефолте 150 (%s)" % p["notes"])
p = plan(contours=[{"outer": rect(0, 0, 700, 600)}], min_cut=50)
ok(p["summary"]["tiles"] == 2,
   "C13b: явный min_cut=50 перекрывает дефолт")

# ── C14 (ТЗ 22.07 п.1.2 — центрирование): пролёт между швами 1520,
#    кусок при 2 целых = 145 < 300 → смещение на полплиты: одна
#    целая ПО ЦЕНТРУ простенка, равные куски по 450 ──
holes14 = [rect(0, 0, 400, 600), rect(1940, 0, 2340, 600)]
p = plan(contours=[{"outer": rect(0, 0, 2340, 600), "holes": holes14}],
         mode="openings")
row = sorted(p["inserts"], key=lambda t: t["x"])
ok([(t["x"], t["w"]) for t in row] ==
   [(410.0, 450.0), (870.0, 600.0), (1480.0, 450.0)],
   "C14: целая по центру простенка, равные куски 450 (%s)"
   % [(t["x"], t["w"]) for t in row])

# ── C21 (1.2, сдвиг): пролёт 1580 — кусок при 2 целых 175 < 300 →
#    сдвиг: 1 целая по центру, куски 480 ──
p = plan(contours=[{"outer": rect(0, 0, 2400, 600),
                    "holes": [rect(0, 0, 400, 600),
                              rect(2000, 0, 2400, 600)]}],
         mode="openings")
row = sorted(p["inserts"], key=lambda t: t["x"])
ok([(t["x"], t["w"]) for t in row] ==
   [(410.0, 480.0), (900.0, 600.0), (1510.0, 480.0)],
   "C21: сдвиг на полплиты, куски 480 (%s)"
   % [(t["x"], t["w"]) for t in row])

# ── C22 (1.2, БЕЗ сдвига): пролёт 1830 — куски при 2 целых ровно
#    300 ≥ 300 → блок из 2 целых по центру, куски 300 ──
p = plan(contours=[{"outer": rect(0, 0, 2650, 600),
                    "holes": [rect(0, 0, 400, 600),
                              rect(2250, 0, 2650, 600)]}],
         mode="openings")
row = sorted(p["inserts"], key=lambda t: t["x"])
ok([(t["x"], t["w"]) for t in row] ==
   [(410.0, 300.0), (720.0, 600.0), (1330.0, 600.0), (1940.0, 300.0)],
   "C22: куски 300 — без сдвига, блок 2 целых по центру (%s)"
   % [(t["x"], t["w"]) for t in row])

# ── C23 (2.5): юзер-точка вертикального руста — ось шва по X=1220,
#    слева целые от угла, справа целые от правого угла, кусок у шва ──
p = plan(contours=[{"outer": rect(0, 0, 3040, 600)}],
         mode="openings", vjoints=[1220.0])
row = sorted(p["inserts"], key=lambda t: t["x"])
ok([(t["x"], t["w"]) for t in row] ==
   [(0.0, 600.0), (610.0, 600.0), (1225.0, 595.0), (1830.0, 600.0),
    (2440.0, 600.0)],
   "C23: руст через точку, слева от угла, справа от угла с куском "
   "у шва (%s)" % [(t["x"], t["w"]) for t in row])

# ── C24: дефолтный режим движка = основной (центрирование/от угла),
#    как ТЗ 22.07 — без mode результат совпадает с mode=openings ──
pa = cladding_plan({"tile": {"w": 600, "h": 600},
                    "gap": {"v": 10, "h": 10}, "datum": 0.0,
                    "contours": [{"outer": rect(0, 0, 2340, 600),
                                  "holes": holes14}]})
ok([(t["x"], t["w"]) for t in sorted(pa["inserts"],
                                     key=lambda t: t["x"])] ==
   [(410.0, 450.0), (870.0, 600.0), (1480.0, 450.0)],
   "C24: дефолтный режим — центрирование по ТЗ 22.07")

# ── C15 (В5-B, n=1): пролёт между швами 860 — целых 0 в центре, два
#    равных подрезных (250+600)/2=425 ──
holes15 = [rect(0, 0, 400, 600), rect(1280, 0, 1680, 600)]
p = plan(contours=[{"outer": rect(0, 0, 1680, 600), "holes": holes15}],
         mode="openings")
row = sorted(p["inserts"], key=lambda t: t["x"])
ok([(t["x"], t["w"]) for t in row] == [(410.0, 425.0), (845.0, 425.0)],
   "C15: простенок со швами — два равных подрезных по 425 (%s)"
   % [(t["x"], t["w"]) for t in row])

# ── C17 (П4 по вертикали, ПЕРЕПИСАН по фидбэку Германа 23.07): окно-
#    «лента» в правой половине, не выровнено по рядам — столбец У
#    окна отступает от него на гориз. руст (490/540), а столбец МИМО
#    окна идёт ЦЕЛЫМИ рядами (прежде низ/верх окна резал его на
#    500+100 с выпадением куска 100 < min_cut — «двойные плитки» и
#    дыры сбоку окон на скринах 23.07) ──
p = plan(contours=[{"outer": rect(0, 0, 1220, 1210),
                    "holes": [rect(610, 500, 1220, 660)]}])
t17 = sorted([(t["x"], t["y"], t["w"], t["h"]) for t in p["inserts"]])
ok(t17 == [(0.0, 0.0, 600.0, 600.0), (0.0, 610.0, 600.0, 600.0),
           (610.0, 0.0, 600.0, 490.0), (610.0, 670.0, 600.0, 540.0)],
   "C17: столбец мимо окна целый, у окна — отступы gh (%s)" % t17)

# ── C18 (П3): origin.x — вертикальный руст через точку: сетка
#    столбцов от x=305 (модульно, клип краями контура) ──
p = plan(contours=[{"outer": rect(0, 0, 3040, 1210)}],
         origin={"x": 305.0, "y": 0.0})
xs18 = sorted({(t["x"], t["w"]) for t in p["inserts"]})
ok(xs18 == [(0.0, 295.0), (305.0, 600.0), (915.0, 600.0),
            (1525.0, 600.0), (2135.0, 600.0), (2745.0, 295.0)],
   "C18: столбцы от точки верт. руста (%s)" % xs18)
ok(p["summary"]["tiles"] == 12 and p["summary"]["cut"] == 4,
   "C18: 2 ряда × (4 целых + 2 подрезки)")

# ── C19 (П3): origin.y — синоним датума (низ первого ряда) ──
pa = plan(contours=[{"outer": rect(0, 0, 610, 1210)}],
          origin={"y": 300.0})
pb = plan(contours=[{"outer": rect(0, 0, 610, 1210)}], datum=300.0)
ok([(t["y"], t["h"]) for t in pa["inserts"]] ==
   [(t["y"], t["h"]) for t in pb["inserts"]],
   "C19: origin.y эквивалентен datum")

# ── C16: агрегация одинаковых notes со счётчиком ──
p = plan(contours=[{"outer": rect(0, 0, 700, 1820)}])   # 3 ряда по 90
ok(any("90 < min_cut (×3)" in n for n in p["notes"]),
   "C16: одинаковые пропуски схлопнуты (×3) (%s)" % p["notes"])

# ── C12: окно уже camня в простенке — вставка min(rem, w) не шире w ──
p = plan(contours=[{"outer": rect(0, 0, 1400, 600),
                    "holes": [rect(100, 0, 200, 600),
                              [[1300, 0], [1350, 0], [1350, 600],
                               [1300, 600]]]}],
         mode="openings")
ok(all(t["w"] <= 600 + 1e-6 for t in p["inserts"]),
   "C12: ни один камень не шире 600")

# ── C25 (Г1): от угла к окну с подрезкой <300 — плитка перед
#    подрезкой в половину (300), подрезка увеличена (rem+300) ──
p = plan(contours=[{"outer": rect(0, 0, 3000, 600),
                    "holes": [rect(1500, 0, 2100, 600)]}],
         mode="openings")
row = sorted(p["inserts"], key=lambda t: t["x"])
ok([(t["x"], t["w"]) for t in row] ==
   [(0.0, 600.0), (610.0, 300.0), (920.0, 570.0),
    (2110.0, 580.0), (2700.0, 300.0)],
   "C25: Г1 — полуплитка перед подрезкой с обеих сторон окна (%s)"
   % [(t["x"], t["w"]) for t in row])

# ── C26 (Г3): точка горизонтального руста = низ руста; к русту
#    снизу подрезка не нужна (кратно), выше — панели стандартной
#    высоты от py+gh ──
p = plan(contours=[{"outer": rect(0, 0, 1220, 2440)}],
         mode="openings", hjoints=[1210.0])
t26 = sorted({(t["y"], t["h"]) for t in p["inserts"]})
ok(t26 == [(0.0, 600.0), (610.0, 600.0), (1220.0, 600.0),
           (1830.0, 600.0)],
   "C26: пояса от руста 1210 — панели стандартной высоты (%s)" % t26)

# ── C27 (Г3): руст не кратен сетке — к русту облицовка приходит С
#    ПОДРЕЗКОЙ (390), выше руста снова целые ──
p = plan(contours=[{"outer": rect(0, 0, 610, 2440)}],
         mode="openings", hjoints=[1000.0])
t27 = sorted({(t["y"], t["h"]) for t in p["inserts"]})
ok(t27 == [(0.0, 600.0), (610.0, 390.0), (1010.0, 600.0),
           (1620.0, 600.0), (2230.0, 210.0)],
   "C27: подрезка 390 к русту, панели 600 выше (%s)" % t27)

# ── C28 (Г2): окно уже камня — над окном и в ряду окна камень
#    шириной ровно в пролёт окна (не шире проёма) ──
p = plan(contours=[{"outer": rect(0, 0, 1400, 1210),
                    "holes": [rect(500, 0, 900, 600)]}],
         mode="openings")
w28 = sorted({(t["x"], t["w"]) for t in p["inserts"]
              if near(t["x"], 500)})
ok(w28 == [(500.0, 400.0)],
   "C28: камень над узким окном = ширине окна (%s)" % w28)

# ═══ Фидбэк Германа 23.07 (первый живой прогон) — модель его
#     фасада: контур 3000×3000, окно 600×1400 по центру, русты 8 ═══
FB = {"tile": {"w": 600, "h": 600}, "gap": {"v": 8, "h": 8},
      "datum": 0.0,
      "contours": [{"outer": rect(0, 0, 3000, 3000),
                    "holes": [rect(1200, 1000, 1800, 2400)]}]}

# ── C29: боковые столбцы у окна НЕ дробятся его низом/верхом:
#    во всех рядах, кроме верхнего (568), они целые 600 ──
p = cladding_plan(dict(FB))
side = [t for t in p["inserts"] if t["x"] + t["w"] <= 1200 + 1e-6 or
        t["x"] >= 1800 - 1e-6]
ok(all(near(t["h"], 600) or near(t["h"], 568) for t in side),
   "C29: сбоку от окна только целые ряды и верхняя подрезка (%s)"
   % sorted({round(t["h"]) for t in side}))
ok(not any(near(t["h"], 600 - 8) or t["h"] < 400 for t in side),
   "C29b: «двойных» низких плиток сбоку окна нет")

# ── C30: под окном — подрезка по высоте к низу окна с рустом
#    (608→992−8: h=384), над окном — от верха окна с рустом ──
under = [t for t in p["inserts"]
         if near(t["x"], 1200) and t["y"] < 1000]
ok([(t["y"], t["h"]) for t in sorted(under, key=lambda t: t["y"])] ==
   [(0.0, 600.0), (608.0, 384.0)],
   "C30: под окном целая + подрезка 384 к низу окна (%s)"
   % [(t["y"], t["h"]) for t in under])
over = [t for t in p["inserts"]
        if near(t["x"], 1200) and t["y"] > 2400]
ok(all(t["y"] >= 2408 - 1e-6 for t in over),
   "C30b: над окном камни от верха окна + руст (%s)"
   % [(t["y"], t["h"]) for t in over])
# остаток 16 мм над верхом окна — СТАВИТСЯ (В12, ответ Дениса 24.07:
# принцип раскладки; полоску позже прикроет отлив) + информ-note
ok(any(near(t["x"], 1200) and near(t["y"], 2408) and near(t["h"], 16)
       for t in p["inserts"]),
   "C30c: мелкий кусок 16 мм над окном ставится (В12)")
ok(any("мелкая подрезка по высоте 16" in n for n in p["notes"]),
   "C30d: информативная note про мелкий кусок (%s)" % p["notes"])

# ── C31 (соосность 23.07): клик вертикального руста В ГРАНЬ окна
#    схлопывается с авто-швом грани — план идентичен плану без
#    vjoints (нет сдвоенного руста 12 мм и кромок ±4) ──
pv = cladding_plan(dict(FB, vjoints=[1200.0]))
ok(sorted((t["x"], t["y"], t["w"], t["h"]) for t in pv["inserts"]) ==
   sorted((t["x"], t["y"], t["w"], t["h"]) for t in p["inserts"]),
   "C31: vjoint в грань окна = авто-шов, план не меняется")

# ── C32: vjoint в ПРОСТЕНКЕ (не у окна) работает как ось шва ──
pw = cladding_plan(dict(FB, vjoints=[300.0]))
ok(any(near(t["x"] + t["w"], 296) for t in pw["inserts"]) and
   any(near(t["x"], 304) for t in pw["inserts"]),
   "C32: дальний vjoint остаётся осевым швом")

# ── C33: проём целиком ВНУТРИ одного ряда (ниша): столбец даёт
#    камень И под, и над нишей в одном ряду ──
p = cladding_plan({"tile": {"w": 600, "h": 600},
                   "gap": {"v": 8, "h": 8}, "datum": 0.0,
                   "contours": [{"outer": rect(0, 0, 1800, 600),
                                 "holes": [rect(600, 200, 1200, 400)]}]})
mid = sorted([(t["y"], t["h"]) for t in p["inserts"]
              if near(t["x"], 600)])
ok(mid == [(0.0, 192.0), (408.0, 192.0)],
   "C33: ниша в ряду — камень под и над ней (%s)" % mid)

# ── C34 (мост к AFrame, 24.07): оси вертикальных швов и центры
#    горизонтальных — из стыков камней + граней окна ──
p = cladding_plan(dict(FB))
ok(p["joints_x"] == [604.0, 1196.0, 1804.0, 2396.0],
   "C34: оси стоек = стыки столбцов + грани окна ± полруста (%s)"
   % p["joints_x"])
ok(p["rows_y"] == [604.0, 1212.0, 1820.0, 2428.0],
   "C34b: центры горизонтальных швов (%s)" % p["rows_y"])

# ── C35 (04.08, фидбэк Германа): контур из АР не идеально
#    ортогонален — раскладка не должна падать из-за долей миллиметра ──
_ref = len(plan(contours=[{"outer": rect(0, 0, 3000, 3000)}])["inserts"])
for _d in (0.01, 0.5, 2.0, 4.9):
    _p = plan(contours=[{"outer": [[0, 0], [3000, _d],
                                   [3000, 3000], [0, 3000]]}])
    ok(len(_p["inserts"]) == _ref and
       not any("неортогональ" in n for n in _p["notes"]),
       "C35: отклонение %.2f мм выпрямляется, раскладка идёт (%d/%d)"
       % (_d, len(_p["inserts"]), _ref))

# C36: реально наклонное ребро — по-прежнему честный отказ, но нота
#      называет величину (раньше молчала, и было не понять, что не так)
_p = plan(contours=[{"outer": [[0, 0], [3000, 100.0],
                               [3000, 3000], [0, 3000]]}])
ok(not _p["inserts"] and
   any("неортогональ" in n and "100.0" in n for n in _p["notes"]),
   "C36: наклон 100 мм — отказ с величиной в ноте (%s)" % _p["notes"])

# C37: допуск настраивается — тот же наклон при ortho_tol=150 раскладывается
_p = cladding_plan({"tile": {"w": 600, "h": 600},
                    "gap": {"v": 8, "h": 8}, "datum": 0.0,
                    "ortho_tol": 150.0,
                    "contours": [{"outer": [[0, 0], [3000, 100.0],
                                            [3000, 3000], [0, 3000]]}]})
ok(len(_p["inserts"]) == _ref,
   "C37: ortho_tol=150 выпрямляет наклон 100 мм (%d)"
   % len(_p["inserts"]))

# C38: выпрямление НЕ ломает сложную ступенчатую форму (разная
#      высотность — жалоба «не раскладывает сложные области»)
_step = [[0, 0], [4000, 0.3], [4000.2, 1200], [3000, 1200.4],
         [2999.8, 2400], [1000, 2400.2], [1000.3, 3600], [0, 3600]]
_p = plan(contours=[{"outer": _step}])
ok(len(_p["inserts"]) > 0 and
   not any("неортогональ" in n for n in _p["notes"]),
   "C38: ступенчатый контур с шумом раскладывается (%d камней)"
   % len(_p["inserts"]))

print("cladding_plan: %d проверок OK" % _n)
