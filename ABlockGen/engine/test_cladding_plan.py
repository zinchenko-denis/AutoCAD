...
def plan(**kw):
    base = {"tile": {"w": 600, "h": 600}, "gap": {"v": 10, "h": 10},
            "datum": 0.0, "mode": "edge", "min_cut": 20.0}
    base.update(kw)
...
# ── C4: «от проёмов» — двери во всю высоту ряда, простенок ──
...
ok([(t["x"], t["w"]) for t in row] ==
   [(0.0, 200.0), (800.0, 600.0), (1410.0, 180.0), (1600.0, 600.0),
    (2800.0, 200.0)],
   "C4: целые от границ проёмов, рез в середине простенка (%s)"
   % [(t["x"], t["w"]) for t in row])

# ── C5: «от проёмов» — ряд НАД проёмами якорей не имеет, от края ──
p = plan(contours=[{"outer": rect(0, 0, 3000, 1210), "holes": holes4}],
         mode="openings")
top = sorted([t for t in p["inserts"] if near(t["y"], 610)],
             key=lambda t: t["x"])
ok(len(top) == 5 and near(top[0]["x"], 0) and near(top[4]["w"], 560),
   "C5: верхний ряд от края, подрезка справа (%s)"
