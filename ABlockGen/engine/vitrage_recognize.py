...
def thermal_tiers(y_lo, y_hi, l1, l2, gap):
    ...
    верхний ряд — остаток. Полный этаж добавляется, пока хвост после него
    ≥ ПОЛЭТАЖА (18.07, лог Алексея: жадное поглощение слило 2995+2845 в
    «ярус» 5845; при этом мелкий хвост «Пробы 2» 3145=2995+150 остаётся
    единым — 150 < h2/2). → [(y0, y1)] снизу вверх."""
    ...
    b = l2 + gap
    while y_hi - b > h2 + gap + h2 / 2.0 - EPS:   # хвост после полного ≥ h2/2
        tiers.append((b, b + h2))
        b += h2 + gap
...
def recognize(req):
    ...
    # ИМЯ стороны = сторона ОСТРИЯ (18.07d, доказано vis-наборами
    # состояний его блока «Створки»: «Левое поворотное» рисует галочку
    # остриём ВЛЕВО/концами вправо; все 5 створок эталона «Пробы 2.1» —
    # «Правое поворотное» при галочках АР остриём вправо). До 18.07d
    # мапилось от концов («концы слева → Левое») — зеркально, фидбэк
    # Алексея по #64 «строится левое открывание».
    ...
    if fill_name or sash_name:
        ...
        for v in (req.get("vees") or []):
            ...
            if abs(e1x - e2x) <= abs(e1y - e2y) * 0.5:
                ...
                # имя = сторона ОСТРИЯ (18.07d, блок «Створки» Алексея)
                apex_left = ax < (e1x + e2x) / 2.0
                vees_side.append((cxv, cyv, apex_left))
